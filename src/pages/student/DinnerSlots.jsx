...
export default function DinnerSlots() {
    ...
    // In tomorrow-booking mode (past 8:30 PM), dinner is OPEN for booking tomorrow.
    // In today mode, dinner closes after 8:30 PM.
    // So: if isTomorrow => open; else check isMealClosed('dinner')
    const isLocked = isTomorrow ? false : isMealClosed('dinner');

    async function fetchData() {
        try {
            setLoading(true);
            const [all, mine, menus] = await Promise.all([
                bookingService.getSlotDistribution(bookingDate, 'dinner'),
    ...
    async function handleConfirm() {
        ...
        try {
            if (myBooking) {
                await bookingService.updateSlot(myBooking.id, user.id, bookingDate, 'dinner', pendingSlot.value);
                toast.success('Slot changed!', `Your dinner slot is now ${pendingSlot.label}`);
            } else {
                await bookingService.createBooking(user.id, bookingDate, 'dinner', pendingSlot.value);
    ...
    async function handleCancel() {
        ...
        setSubmitting(true);
        try {
            await bookingService.cancelBooking(myBooking.id, 'dinner');
            toast.success('Booking cancelled', 'Your dinner booking has been removed.');
            setMyBooking(null);
            await fetchData();
        } catch (err) {
        ...
        <div className="space-y-6 animate-fade-in">
            ...
            {isLocked ? (
                <div className="flex items-center gap-3 p-3.5 rounded-xl bg-surface border border-border">
                    ...
                    <div>
                        <p className="text-sm font-semibold text-text-secondary">Dinner slot changes locked for today</p>
                        <p className="text-xs text-text-muted mt-0.5">After 8:30 PM you can book for tomorrow's dinner.</p>
                    </div>
                </div>
            ) : (
                <div className={`flex items-center gap-3 p-3.5 rounded-xl border ${isTomorrow ? 'bg-green-50 border-green-200' : 'bg-blue-50 border-blue-200'
                    ...
                    <p className={`text-sm font-semibold ${isTomorrow ? 'text-green-700' : 'text-blue-700'}`}>
                        {isTomorrow
                            ? "Booking open for tomorrow's dinner"
                            : 'Booking open — window closes at 8:30 PM'}
                    </p>
                </div>
            )}
                    ...
                    <div className="p-4 rounded-xl bg-primary/5 border border-primary/15">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-sm font-semibold text-text">
                                {isTomorrow ? "Tomorrow's Dinner Menu" : "Tonight's Dinner Menu"}
                            </span>
                        </div>
                                ...
                                {DINNER_SLOTS.map((slot) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            disabled={isLocked || submitting || isFull}
                        ...
                        {loading ? (
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="bg-surface-hover rounded-xl p-3 space-y-2">
                                    ...
                                    {profile?.preferred_dinner_slot && profile.preferred_dinner_slot !== myBooking.slot_time && (
                                        <div className="flex items-center justify-between text-xs pt-1 border-t border-border">
                                            <span className="text-text-muted font-medium">Preferred</span>
